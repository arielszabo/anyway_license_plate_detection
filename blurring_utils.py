...
def find_in_img(image_frame, darknet_model, class_label=['vehicle registration plate'], thresh=0.5,
    ...
    for category, score, bounds in results:
        ...
        if category.lower() in [label.lower() for label in class_label]:
            x, y, w, h = bounds
...
def get_video_params(capture_vid):
    ...
    print(capture_vid.get(cv2.CAP_PROP_FORMAT))

    return width, height, fps, fourcc


def find_all(video_path, darknet_model, thresh,
             class_label=['vehicle registration plate'], start_frame=0,
             end_frame=None):
    ...
    while video_capture.isOpened():
        ...
        if ret:
            if counter > start_frame and counter < end_frame:
                ...
                frame_results_bounds = find_in_img(image_frame=frame,
